...
def index():
    ...
    if request.method=="POST":
        ...
        for page in range(3):
            ...
            response = requests.get(url_proxy,proxies={"https": proxy_list1},timeout=90)
        ...
        response = requests.get(base_url, params=params, headers=headers, proxies={"https": proxy_list1},timeout=90)

        while response.status_code!=200:
            selected_proxy = random.sample(proxy_list, 5)
            print(f"Using https://{selected_proxy} proxy")
            response = requests.get(base_url, params=params, headers=headers, proxies={"https": selected_proxy},timeout=90)
            time.sleep(np.random.randint(5, size=(1))[0])
        content = BeautifulSoup(response.content, 'html.parser')
        ...
        for name in title:
            ...
            # Scraped results
            response1 = requests.get(base_url, params=params, headers=headers, proxies={"https": proxy_list1},timeout=90)
            ...
            if adr==None:
                ...
                response2 = requests.get(base_url, params=params, headers=headers, proxies={"https": selected_proxy2},timeout=90)
                while response2.status_code != 200:
                    ...
                    response2 = requests.get(base_url, params=params, headers=headers,
                                             proxies={"https": selected_proxy2},timeout=90)
                    time.sleep(np.random.randint(5, size=(1))[0])
                # time.sleep(2)
